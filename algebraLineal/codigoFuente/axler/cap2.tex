...
\setcounter{myteo}{22}
%-------------------- 2.23 teorema
\begin{myteo}[La longitud de la lista linealmente independiente es  $\boldmath \leq$ que  la longitud de la lista generadora]\;\\\\
...
\begin{myteo}[La lista generadora contiene un base]\,\\\\
	...
	Demostración.-\; Suponga que $v_1,\ldots,v_n$ genera $V$. Queremos eliminar algunos de los vectores de  $v_1,\ldots,v_n$ para que los vectores restantes formen una base de $V$. \\
	Sea $B=v_1,\ldots,v_n$, de donde realizamos un bucle con las siguientes condiciones:
	\begin{enumerate}[\textbf{Paso} 1.]
	    \item Si $v_1=0$, eliminamos $v_1$ de $B$. Si $v_1\neq 0$ entonces no cambiamos $B$.
	    \item Si $v_j$ esta en $\span(v_1,\ldots,v_{j-1})$, eliminamos $v_j$ de $B$. Si $v_j$ no está en $\span(v_1,\ldots,v_j)$, entonces no cambiamos $B$ (lema 2.21). 
	\end{enumerate}
