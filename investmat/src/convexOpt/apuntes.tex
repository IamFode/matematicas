...
\begin{teo} Sea $f$ convexa, $x_0$ un mínimo local de $f$. Entonces, $x_0$ es un mínimo global.
    ...
    Demostración.-\; Sea $\delta>0$ tal que $f(z)\geq f(x_0),\; \forall z\in B(x_0,\delta)\cap Dom$. Lo que quiero demostrar es que la desigualdad dada, no sólo es cierta para todos los puntos de la bola si no para todos los puntos cualquiera. Así, que tomamos un $y\in Dom$; la idea será:
    \begin{center}
	\begin{tikzpicture}[>=Triangle,scale=.6]
	    ...
	    \draw (-1,-.5) -- (2,1.2);
	    \fill (-.24,-.08) circle (1.5pt)node[below right]{\scriptsize$z$};
	\end{tikzpicture}
    \end{center}
	...
	\begin{tikzpicture}[>=Triangle,scale=.5]
	    ...
	    \draw[dashed](0,0)--(0,2)circle (2pt);
	    \draw[dashed](0,2)--(5,2);
	    \fill (4,1.5) circle (3pt);
	    \draw[dashed](1,3)--(4,1.5);
	    %crear flecha curva
	    \draw[->] (2,2.5) to [out=0,in=188] (4,3);
	    \foreach \x/\xtext/\y in {0/ x_0/0,4/ y/0}
	    {
	    \draw (\x,.1) -- (\x,-.1) node[below] {$\xtext$};
	    }
	    \draw[>={Bracket[width=3mm,line width=.5pt,length=.5mm]},<->] (-.5, 0) -- (.5, 0);
	\end{tikzpicture}
    \end{center}
