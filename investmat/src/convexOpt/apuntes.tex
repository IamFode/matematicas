...
\begin{teo} $f$ es convexa sii $\text{Epi}(f)$ es convexo.
	...
	\begin{tikzpicture}[>=Triangle,scale=.6]
	    ...
	    \fill[draw](-2.1,3.5)node[above left]{\scriptsize$(x,t)$} circle (2pt) -- (0,3.915) circle (2pt)node[above,rotate=10]{\tiny$\lambda t+(1-\lambda) s$} -- (2.74,4.5)node[above]{\scriptsize$(y,s)$} circle (2pt);
	    \fill[draw](-2.1,1.5) circle (2pt) -- (0,1.915) circle (2pt) -- (2.74,2.5) circle (2pt);
	    ...
	    \foreach \x/\xtext/\y in {-2.1/\scriptsize x/-.1,0/\tiny \lambda x (1-\lambda)y/-.1,2.74/\scriptsize y/-.1}
	    {
	    \draw (\x,-1.2) -- (\x,-.8) node[below,yshift=-.2 cm] {\scriptsize$\xtext$};
	    }
	\end{tikzpicture}
