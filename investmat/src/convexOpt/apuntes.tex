...
¿Por qué el cero está ($\in$) y no es?. Respondamos con el siguiente ejemplo:
...
\begin{ejem} $f(x)=|x|$
	...
	\begin{tikzpicture}[>=Triangle,scale=.5]
	    ...
	    \draw(0,0)--(0,5);
	    \draw(-4,1.5)--(4,1.5);
	    \draw[blue!70,domain=-3:3,smooth,variable=\x,fill] plot ({\x},{1.5})--(0,1.5)circle(3pt);
	    \draw[](-3,4.5)--(0,1.5)--(3,4.5);
	\end{tikzpicture}
    \end{center}
    Cómo es un mínimo la recta horizontal estará por debajo y eso quiere decir que el cero está en ese mínimo y esa recta horizontal pertenece al subdiferencial, pues el punto cero es un mínimo global; gracias a que la función esta por encima y que es convexa.
    \begin{itemize}
	\item Si $x\neq 0,\quad  \partial f(x)=\left\{f'(x)\right\}$.
	\item Si $x=0, \quad \partial f(0)=[-1,1]$. Todos los puntos en este intervalo me dan rectas tangentes. Osea,
	$g(x)=0+\alpha(x-0)=\alpha x$ es un subestimador afín $\forall \alpha \in [-1,1]$.
    \end{itemize}
\end{ejem}
...
\end{tcolorbox}
\begin{center}
    \begin{tikzpicture}[>=Triangle,scale=.5,rotate=130]
	\draw[pattern=north west lines, pattern color=blue!20, draw=white,rotate around={0:(0,0)}] (0,0) -- (5cm,1.1) -- (1,4.5cm) -- cycle;
	\draw[blue](0,0)--(1,4.5);
	\draw[blue](0,0)--(5,1);
	\draw[red](2,1)node[below left]{$C$};
    \end{tikzpicture}
\end{center}
...
\begin{center}
	...
	\draw[blue](0,0)--(1,4.5);
	\draw[blue](0,0)--(5,1);
	\draw[red](3,.5)node[below left]{$C$};
	\draw[red,fill](0,0)circle(2pt)node[right]{$x_0$}--(2,2.5)circle(2pt)node[left]{$x$}node[above,xshift=7mm]{\tiny$(x-x_0)$};
	\draw[red,->](0,0)--(2,2.5);
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[>=Triangle,scale=.5,rotate=130]
	\draw[pattern=north west lines, pattern color=blue!20, draw=white,rotate around={0:(0,0)}] (0,0) -- (5cm,1.1) -- (1,4.5cm) -- cycle;
	\draw[pattern=north west lines, pattern color=red!20, draw=white,rotate around={180:(0,0)}] (0,0) -- (5cm,1.1) -- (1,4.5cm) -- cycle;
	\draw[blue](-1,-4.5)--(1,4.5);
	\draw[blue](-5,-1)--(5,1);
	\draw[red](3,.5)node[below left]{$C$};
	\draw[red](-4.5,-5.5)node[below left]{$\mathcal{N}_C(x_0)$};
	\draw[red,fill](0,0)circle(3pt)node[above]{$x_0$}--(2,2.5)circle(3pt)node[left]{$x$}node[above,xshift=7mm]{\tiny$(x-x_0)$};
	\draw[red,->](0,0)--(2,2.5);
    \end{tikzpicture}
\end{center}
...
% -------------------- PROPIEDAD 3.2
\begin{prop}[Propiedades operacionales del subdiferencial]\; Sean $f_1,f_2$ convexas.
    \begin{enumerate}[1.]
...
\begin{center}
    \begin{tikzpicture}[>=Triangle,scale=.5,rotate=130]
	\draw[pattern=north west lines, pattern color=blue!20, draw=white,rotate around={0:(0,0)}] (0,0) -- (5cm,1.1) -- (1,4.5cm) -- cycle;
	\draw[pattern=north west lines, pattern color=yellow!30, draw=white,rotate around={180:(0,0)}] (0,0) -- (5cm,1.1) -- (1,4.5cm) -- cycle;
	\draw[blue](-1,-4.5)--(1,4.5);
	\draw[blue](-5,-1)--(5,1);
	\draw[red](3,.5)node[below left]{$C$};
	\draw[red](-4.5,-5.5)node[below left]{$\mathcal{N}_C(x_0)$};
	\draw[domain=-4.5:4,smooth,variable=\x,rotate=-20] plot ({\x},{-.1*\x*\x});
	\draw[domain=-3.5:3.5,smooth,variable=\x,rotate=-20] plot ({\x},{-.1*\x*\x-1.5});
	\draw[red,domain=-4:4,smooth,variable=\x,rotate=-10] plot ({\x},{-.2*\x});
	\fill[] (0,0) circle (4pt);
	\draw[->,red] (0,0) -- (-1,-3);
    \end{tikzpicture}
\end{center}
...
\section{La función conjugada (Conjugada de Fenchel)}

% -------------------- DEFINICIÓN 3.13
\begin{def.}
    Sea $f:\mathbb{R}^n\to \mathbb{R}$ una función, se define su conjugada de Fenchel $f^*:\mathbb{R}^n \to \mathbb{R}$ dada por:
    $$
    \begin{array}{rcl}
	f^*(y)&=&\sup\left\{y^Tx-f(x): x\in \dom(f)\right\}\\\\
	      &=&\sup\left\{\langle y,x\rangle-f(x): x\in \dom(f)\right\}\\\\
	      &=&\sup\left\{\langle y,x\rangle-f(x): x\in \mathbb{R}^n\right\}.
    \end{array}
    $$
    Esto, si $x\notin \dom(f)$. Entonces, $y^Tx-f(x)=-\infty$.
\end{def.}

Entendamos geométricamente:

\begin{center}
    \begin{tikzpicture}[>=Triangle,scale=1.2]
	\tikzstyle{every node}=[font=\scriptsize]

	\draw[opacity=.5](-2,0)--(1.5,0)node[below]{};
	\draw[opacity=.5](0,-1)--(0,1.5)node[below]{};

	\draw[domain=-1.5:1,smooth,variable=\x] plot ({\x},{1.5*\x*\x*\x*\x+\x*\x*\x-1.5*\x*\x+.4}); 
	\draw[blue](-1.7,-.9)--(1.2,1.2)node[right]{$y^Tx=0$};
	\draw[red](-1.3,-1.1)--(1.4,.85);
	\fill[red](.62,.29) circle (1pt);
	\fill[red](0,-.165) circle (1pt)node[right]{$-f^*(y)$};
