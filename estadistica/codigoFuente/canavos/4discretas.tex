...
Que es la función de probabilidad de la distribución puntual o Bernoulli.\\\\
...
De donde se sabe que $p(y; m,p) = \dfrac{m!}{(m-y)!y!}p^{y}(1-p)^{n-y}$   es la función de probabilidad de una variable aleatoria binomial $Y$ con parámetros $m=n-1$ y $p$; de ésta manera $\sum_{y=0}^m p(y;m,p) = 1$ y la media de una variable  aleatoria binomial es:
$$E(X)=\mu = np.$$
