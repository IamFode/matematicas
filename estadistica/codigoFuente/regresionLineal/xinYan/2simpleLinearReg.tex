...
\begin{teo}
	...
	Demostración.-\; Dado que, $\E(X)=\mu$ y $\Var(X)=\sigma^2$. Entonces para $Y=a_1X_i+a_2X_2+\ldots+a_nX_n$ tenemos
	$$E(y)=\sum_{i=1}^n a_i\mu_i\quad \mbox{y}\quad Var(Y)=\sum_{i=1}^n a_i^2\sigma_i^2.$$, luego
	$$
	\begin{array}{rcl}
	    \Var(b_0) &=& \Var(\overline{y}-b_1\overline{x})\\\\
		      &=& \Var(\overline{y})+(\overline{x})^2\Var(b_1)\\\\
		      &=& \displaystyle\sum_{i=1}^n \dfrac{1}{n^2}\sigma^2 + (\overline{x})^2\Var(b_1)\\\\
