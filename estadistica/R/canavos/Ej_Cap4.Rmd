...
Respuesta.- La probabilidad de tener instrumentos defectuosos es de $4/40=0.1$ por lo que el número promedio de ocurrencias de las muestras seleccionadas es $\lambda=8\cdot 0.1=0.8$, de donde 

$$P(X\leq 1) = F(1;0.8) = \sum_{i=0}^1 \dfrac{e^{-0.8}\cdot 0.8^i}{i!} =0.8087921$$
...
Respuesta.- El enunciado no es del todo claro, pero al entender nos dice que nos envió menos 2 unidades, pero se sospecha que puede haber más de dos defectuosos, cuando el cliente de esas 50 unidades selecciona 5 muestras ve que una es defectuosa por lo tanto $p=1/5$ (decimos que selecciono de las 50 porque aún no llega el embarque con las 48 unidades.). Ahora si multiplicamos dicha probabilidad por las 50 unidades nos da el verdadero $\lambda = 1/5*50 = 10$ de donde solo nos queda ver si existe más de 2 unidades defectuosas, es decir,

$$P(X\geq 2) = 1- P(X\leq 1) = 1-F(1;10) = \sum_{i=0}^1\dfrac{e^{-10}\cdot 10^i}{i!} = 0.9995 = 1$$

```{r}
ppois(1,10,lower.tail = FALSE)
```
Por lo que el cliente debe reclamar una indemnización al fabricante.
...
### a)
Si la lista de vontantes se encuentra igualmente dividida por sexo. ¿cuál es la probabilidad de tener una lista que contenga a 20 hombres y cinco mujeres?
