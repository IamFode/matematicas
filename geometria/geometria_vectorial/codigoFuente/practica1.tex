...
%----------33.
\item \textbf{\boldmath Mostrar que si $\vec{b}_1$ y $\vec{b_2}$ están en direcciones opuestas, entonces $\Comp_{\vec{b}_1}\vec{a}=-\Comp_{\vec{b}_2}\vec{a}$.\\\\
    Demostración.-}\; Por hipótesis se tiene que $b_1=-b_2$, de donde se tiene,
    $$\Comp_{\vec{b}_1}\vec{a}=\dfrac{\vec{a}\circ \vec{b}_1}{\|\vec{b}_1\|\|\vec{b}_1\|}=\dfrac{\vec{a}\circ (-\vec{b}_2)}{\|-\vec{b}_2\|\|-\vec{b}_2\|}=\dfrac{-\left(\vec{a}\circ \vec{b}_2\right)}{\left(-\|\vec{b}_2\|\right)\left(-\|\vec{b}_2\|\right)}=-\dfrac{\vec{a}\circ \vec{b}_2}{\|\vec{b}_2\|\|\vec{b}_2\|}=-\Comp_{\vec{b}_2}\vec{a}.$$\\

%----------34.
\item \textbf{¿ Que condiciones sobre $\vec{a},  \vec{b}$ y $\vec{c}$ nos aseguran que $\vec{a},  \vec{b}$ y $\vec{c}$ son los lados de un triángulo?.\\\\
    Respuesta.-}\; Para que $\vec{a},  \vec{b}$ y $\vec{c}$ sean los lados de un triángulo, se debe cumplir que $$\vec{a}<\vec{b}+\vec{c} \quad \mbox{y}\quad \vec{a}>\vec{b}-\vec{c}.$$\\ 
...
%----------37.
\item \textbf{Si las diagonales de un paralelogramo son ortogonales, entonces demostrar que el paralelogramo es un rombo.\\\\
    Demostración.-}\; Sean $\vec{a},\vec{b}\in V_n$. Debemos que encontrar que la longitud de los cuatro lados de un paralelogramo es igual. Es decir, demostraremos que
    $$(a+b)\circ (a-b)=0\quad \Rightarrow \quad \|\vec{a}\|=\|\vec{b}\|.$$
    Por las propiedades de producto interno o escalar y por el hecho de que $\|\vec{v}\|^2=\vec{v}\circ \vec{v}$, tenemos 
    $$\begin{array}{rcl}
	\left(\vec{a}+\vec{b}\right)\circ \left(\vec{a}-\vec{b}\right)&=&0\\\\
	\vec{a}\circ \left(\vec{a}-\vec{b}\right)+\vec{b}\circ \left(\vec{a}-\vec{b}\right)&=&0\\\\
	\vec{a}\circ \vec{a}-\vec{a}\circ \vec{b}+\vec{b}\circ \vec{a}-\vec{b}\circ \vec{b}&=&0\\\\
	\|\vec{a}\|^2-\vec{a}\circ \vec{b}+\vec{a}\circ \vec{b}-\|\vec{b}\|^2&=&0\\\\
	\|\vec{a}\|^2-\|\vec{b}\|^2&=&0\\\\
	\|\vec{a}\|^2&=&\|\vec{b}\|^2\\\\
    \end{array}$$
    \vspace{0.5cm}
...
%----------39.
    \item \textbf{Demostrar que las diagonales de un paralelogramo se bisecan.\\\\
	Demostración.-}\; Necesitamos mostrar que las dos diagonales se cortan en sus puntos medios mutuos. Dicho de otra manera, necesitamos mostrar que los puntos medios de AC y BD son, de hecho, el mismo punto. 
	\begin{center}
	    \begin{tikzpicture}[scale=.5]
		\draw[line width=1.2pt](0,0) node[rotate=0,pos=1, left]{$A$}--(4,0) node[rotate=0,pos=1, right]{$B$}--(5,4)node[rotate=0,pos=1, right]{$C$}--(1,4)node[rotate=0,pos=1, left]{$D$}--(0,0)--(5,4);
		\draw[line width=1.2pt](1,4)--(4,0);
	    \end{tikzpicture}
	\end{center}
	Sea $M_1$ el punto medio entre $AC$ y $M_2$ el punto medio entre $BD$. Demostraremos que $\vec{AM_1}=\vec{AM_2}.$\\
	Sabemos que,
	$$\vec{AM_1}=\dfrac{1}{2}\vec{AC}.$$
	Luego, necesitamos usar el hecho que $ABDC$ es un paralelogramo. De donde,
	$$\vec{AB}=\vec{DC}\; \Rightarrow \; \vec{AB}=\dfrac{1}{2}\vec{AB}+\dfrac{1}{2}\vec{AB}=\dfrac{1}{2}\vec{AB}+\dfrac{1}{2}\vec{DC}.$$
	Usamos eso para escribir $\vec{AM_2}$ en términos de $A,B,C,D$.
	$$\vec{AM_2}=\vec{AB}+\dfrac{1}{2}\vec{BD}=\dfrac{1}{2}\vec{AB}+\dfrac{1}{2}\vec{DC}+\dfrac{1}{2}\vec{BD}=\dfrac{1}{2}\left(\vec{AB}+\vec{BD}+\vec{DC}\right)=\dfrac{1}{2}\vec{AC}=\vec{AM_1}.$$\\
