...
%----------12.
\item \textbf{\boldmath Dados dos vectores $\vec{a},\vec{b},\vec{c}$ que satisfacen la condición $\vec{a}+\vec{b}+\vec{c} = \vec{0}$ y sabiendo que $\|\vec{a}\|=3, \|\vec{b}\|=1,\|\vec{a}\|=4$. Calcular $\vec{a}\cdot \vec{b}+\vec{b}\cdot \vec{c}+\vec{a}\cdot \vec{c}$\\\\
...
\item \textbf{\boldmath Dados los puntos $P(3,4)$, $Q(1,1)$ y $R(5,2)$, usar métodos vectoriales para encontrar las coordenadas del cuarto vértice del paralelogramo cuyo lados adyacentes son $\vec{PQ}$ y $\vec{QR}$.\\\\
    Respuesta.-}\; Sea $S$ vértice a encontrar entonces por ser un paralelogramo se tiene que $\vec{QR}=vec{PS}$, por lo tanto $$(1-1)-(5,2)=(3,4)-S \; \Longrightarrow \; S=(3,4)-(-4,-1)$$
    $$S=(7,5)$$
    El gráfico corrobora el resultado.
    \begin{center}
	\begin{tikzpicture}[scale=.7]
	    \draw(3,4)node[above]{R}--(1,1)node[left]{Q}--(5,2)node[below]{R}--(7,5)node[right]{S}--(3,4);
	\end{tikzpicture}
    \end{center}
    \vspace{.5cm}
