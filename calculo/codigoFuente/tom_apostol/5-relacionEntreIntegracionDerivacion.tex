...
\begin{ejem}[Integración de potencias recionales]
    ...
    para cualquier intervalo $[a,b]$. Esta fórmula, demostrada para todo entero $n\geq 0$ conserva su validez para todo entero negativo excepto $n=-1$, que se excluye puesto que el denominador aparece $n+1$. Para demostrar $\int_a^b x^n\; dx = \dfrac{b^{n+1}-a^{n+1}}{n+1}\qquad (n=0,1,2,\ldots)$ para $n$ negativo, basta probar que $P(x)=\dfrac{x^{n+1}}{n+1}$ implica $P'(x)=x^n$ cuando $n$ es negativo $x\neq -1$, lo cual es fácil de verificar derivando $P$ como función racional. Hay que tener en cuenta que si $n$ es negativo se deben excluir aquellos intervalos $[a,b]$ que contienen el punto $x=0.$
\end{ejem}
