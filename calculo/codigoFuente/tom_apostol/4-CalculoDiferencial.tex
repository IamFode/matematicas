...
\begin{teo}
	...
	$$\begin{array}{rcl}
	    \dfrac{(f+g)(x+h)-(f+g)(x)}{h}&=&\dfrac{[f(x+h)+g(x+h)]-[f(x)+g(x)]}{h}\\\\
					  &=&\dfrac{f(x+h)-f(x)}{h}+\dfrac{g(x+h)-g(x)}{h}
	\end{array}$$
...
\begin{enumerate}[\bfseries 1.]
	...
	Respuesta.-\; Primero encontraremos el mínimo de la función. Para luego establecer el mínimo encontrado e igualar a $24$. Para encontrar el mínimo tomamos la derivada de $f(x)$,
	$$f'(x)=10x-5Ax^{-6}$$
	Luego, igualando a cero y resolviendo tenemos
	$$10x-5Ax^{-6}=0\quad \quad \Rightarrow \quad 10x^7=5A\quad \Rightarrow \quad x=\left(\dfrac{A}{2}\right)^{1/7}.$$
	Por lo que $f(x)$ tiene un mínimo en este punto $x$. Ahora remplazando $x$, $f(x)$  e igualándolo a $24$, obtenemos
	$$\begin{array}{rcl}
	    5\left(\right)^{\frac{2}{7}}+A\left(\dfrac{A}{2}\right)^{-\frac{5}{7}}&=&24\\\\
	    \left(\dfrac{A}{2}\right)^{\frac{2}{7}} \left[5+A\left(\dfrac{A}{2}\right)^{-1}\right]=24\\\\
	    A=2\left(\dfrac{24}{7}\right)^{\frac{7}{2}}.
	\end{array}$$
	\vspace{.5cm}
	...
	Respuesta.-\; Dado que Apostol nos advierte específicamente que tengamos cuidado de que la solución mínima esté en los extremos de $[0,1]$, calculamos el valor en $0$ y $1$,
	$$\begin{array}{rcl}
	    f(0) &=& -\dfrac{1}{3}0^3 + t^2(0) = 0\\\\
	    f(1) &=& -\dfrac{1}{3}1^3 + t^2(1) = t^2-\dfrac{1}{3}.
	\end{array}$$
	Derivando $f(x)$,
	$$f'(x)=-x^2+t^2.$$
	Igualando a $0$, encontramos que si f(x) tiene un extremo en el interior de [0,1], entonces
	$$-x^2+t^2=0\quad \Rightarrow \quad x^2=t^2\quad \Rightarrow \quad x=|t|\quad (\mbox{ya que }x\geq 0).$$
	Luego, evaluando $f(x)$ en $x=|t|$ se tiene,
	$$f(|t|) = -\dfrac{1}{3}t^2|t|+t^2|t|=\dfrac{2}{3}t^2|t|.$$
	Pero esto es mayor o igual a $0$ para todos los valores de $t \in [-1,1]$, por lo que no puede ser el mínimo (ya que sabemos $f(0) = 0$ que es al menos mayor que $f(0)$). Por lo tanto, el mínimo debe ocurrir en los puntos extremos. Es decir,
	$$m(t)=\left\{\begin{array}{rcl}
		0 &\mbox{si }& t^2\geq \dfrac{1}{3}\\\\
		t^2-\dfrac{1}{3} &\mbox{si }& t^2<\dfrac{1}{3}.
	\end{array}\right.$$
	\vspace{.5cm}
	...
	Respuesta.-\; 
	\begin{enumerate}[(a)]
	    \item Reescribimos el término de error como una función por partes (para que podamos considerar la derivada de cada parte),
	    $$\dfrac{|t-x|}{x}=\left\{\begin{array}{ll}
		\dfrac{t-x}{x} & \mbox{si } t \geq x\\\\
		\dfrac{x-t}{x} & \mbox{si } t < x.
	    \end{array}\right.$$
	    Entonces, tomamos la derivada con respecto a $x$, se tiene
	    $$\left(\dfrac{|t-x|}{x}\right)'=\left\{\begin{array}{rcl}
				-\dfrac{t}{x^2} & \mbox{si } t \geq x\\\\
		-\dfrac{t}{x^2} & \mbox{si } t < x.
	    \end{array}\right.$$
	    Pero sabemos que $a > 0$ y dado que ambos $x$ y $t$ están en el intervalo $[a,b]$, tenemos  $x, t > 0$. Por lo tanto, la derivada no es cero en ningún lugar del intervalo (ya que $-\frac{t}{x^2} \neq 0$ y $\frac{t}{x^2} \neq 0$  por lo que $t$ no es cero). Así, $M(t)$ no puede tener un máximo en $(a,b)$, por lo que el máximo debe estar en uno de los puntos extremos, $x = a $ o $x = b$.\\\\ 

	    \item Ahora, queremos encontrar el valor de $t$ el cual $M(t)$ es menor (es decir, cuando el error máximo es menor). De la parte (a), sabemos que para cualquier $t$ fijo, el valor máximo del error debe ocurrir en un punto final, $x = a$ o $x = b$. Como $M(t)$ es la función que devuelve el máximo error, tenemos
	    $$M(t)=\dfrac{|t-a|}{a}\quad \mbox{o}\quad M(t)=\dfrac{|t-b|}{b}.$$
	    Se sabe que este es el máximo error, por lo que 
	    $$M(t)=\dfrac{|t-a|}{a}\quad \mbox{si}\quad \dfrac{t-a}{a}>\dfrac{t-b}{b}$$
	    y
	    $$M(t)=\dfrac{|t-b|}{b}\quad \mbox{si}\quad \dfrac{t-a}{a}>\dfrac{t-b}{b}.$$
	    Luego, ya que $a\leq t \leq b$, entonces $|t-a|=t-a$ y $|t-b|=b-t$. Ahora,
	    $$\dfrac{t-a}{a}>\dfrac{b-t}{b}\quad \Rightarrow \quad t>\dfrac{2ab}{a+b}$$
	    y
	    $$\dfrac{b-t}{b}>\dfrac{t-a}{a}\quad \Rightarrow \quad t<\dfrac{2ab}{a+b}.$$
	    Así, podemos dar una fórmula para $M(t)$,
	    $$M(t)=\left\{\begin{array}{ll}
		\dfrac{t-a}{a} & \mbox{si } t>\dfrac{2ab}{a+b}\\\\
		\dfrac{b-t}{b} & \mbox{si } t<\dfrac{2ab}{a+b}.
	    \end{array}\right.$$
	\end{enumerate}
	Derivando con respecto a $t$, tenemos
	$$M'(t)=\left\{\begin{array}{ll}
	    \dfrac{1}{a} & \mbox{si } t>\dfrac{2ab}{a+b}\\\\
	    -\dfrac{1}{b} & \mbox{si } t<\dfrac{2ab}{a+b}.
	\end{array}\right.$$
	Por el hecho de que $a$ y $b$ son positivos. Es decir, $M'(t)$ es negativo si $t<\dfrac{2ab}{a+b}$ y $M'(t)$ es positivo si $x>\dfrac{2ab}{a+b}$; Entonces $M(t)$ es decreciente para $t<\dfrac{2ab}{a+b}$ y creciente para $t>\dfrac{2ab}{a+b}.$ Por lo tanto $M(t)$ tiene un mínimo en 
	$$t=\dfrac{2ab}{a+b}\quad \Rightarrow \quad t=\dfrac{a+b}{2ab}=\dfrac{1}{2}\left(\dfrac{1}{a}+\dfrac{1}{b}\right).$$\\
...
\begin{enumerate}[\bfseries 1.]
	...
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x}&=&y+\dfrac{1}{y}\\\\
	    \dfrac{\partial f}{\partial y}&=&x-\dfrac{x}{y^2}
	\end{array}$$
	Las derivadas parciales de segundo orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2}&=&\dfrac{\partial}{\partial x}\left(y+\dfrac{1}{y}\right)=0\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial x\partial y}&=&\dfrac{\partial}{\partial x}\left(\dfrac{\partial f}{\partial y}\right)=\dfrac{\partial}{\partial x}\left(x-\dfrac{x}{y^2}\right)=1+\dfrac{1}{y^2}\\\\
	    f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y\partial x}&=&\dfrac{\partial}{\partial y}\left(\dfrac{\partial f}{\partial x}\right)=\dfrac{\partial}{\partial y}\left(y+\dfrac{1}{y}\right)=1+\dfrac{1}{y^2}\\\\
	    \dfrac{\partial^2 f}{\partial y^2}&=&\dfrac{\partial}{\partial y}\left(x-\dfrac{x}{y^2}\right)=\dfrac{2x}{y^3}
	\end{array}$$
	Luego, vemos que $f_{1,2}(x,y)=f_{2,1}(x,y)$.
	$$f_{1,2}(x,y)=\dfrac{\partial^2f}{\partial x \partial y}=1+\dfrac{1}{y^2}=\dfrac{\partial^2 f}{\partial y \partial x}=f_{2,1}(x,y).$$\\

    %------------------- 4.
    \item $f(x,y)=\sqrt{x^2+y^2}$.\\\\
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x}&=&\dfrac{1}{2}\left(\dfrac{1}{\sqrt{x^2+y^2}}\right)2x=\dfrac{x}{\sqrt{x^2+y^2}}\\\\
	    \dfrac{\partial f}{\partial y}&=& \dfrac{1}{2}\left(\dfrac{1}{\sqrt{x^2+y^2}}\right)2y=\dfrac{y}{\sqrt{x^2+y^2}}
	\end{array}$$
	Las derivadas parciales de segundo orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2}&=&\dfrac{\partial}{\partial x}\left(\dfrac{x}{\sqrt{x^2+y^2}}\right)=\dfrac{\sqrt{x^2+y^2}-\dfrac{x^2}{\sqrt{x^2+y^2}}}{x^2+y^2}=\dfrac{y^2}{\left(x^2+y^2\right)^{\frac{3}{2}}}\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial x\partial y}&=&\dfrac{\partial}{\partial x}\left(\dfrac{y}{\sqrt{x^2+y^2}}\right)=\dfrac{-xy}{\sqrt{\left(x^2+y^2\right)^{\frac{3}{2}}}}\\\\
	    f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y\partial x}&=&\dfrac{\partial}{\partial y}\left(\dfrac{x}{\sqrt{x^2+y^2}}\right)= \dfrac{-xy}{\sqrt{\left(x^2+y^2\right)^{\frac{3}{2}}}}\\\\
	    \dfrac{\partial^2 f}{\partial y^2}&=&\dfrac{\partial}{\partial y}\left(\dfrac{y}{\sqrt{x^2+y^2}}\right)=\dfrac{x^2}{\sqrt{x^2+y^2}^{\frac{3}{2}}}
	\end{array}$$
	Luego, vemos que $f_{1,2}(x,y)=f_{2,1}(x,y)$.
	$$f_{1,2}(x,y)=\dfrac{\partial^2f}{\partial x \partial y}=\dfrac{-xy}{\sqrt{\left(x^2+y^2\right)^{\frac{3}{2}}}}=\dfrac{\partial^2 f}{\partial y \partial x}=f_{2,1}(x,y).$$\\

    %------------------- 5.
    \item $f(x,y)=\sen\left(x^2y^3\right)$.\\\\
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x}&=&2xy^3\cos\left(x^2y^3\right)\\\\
	    \dfrac{\partial f}{\partial y}&=&3x^2y^2\cos\left(x^2y^3\right)
	\end{array}$$
	Las derivadas parciales de segundo orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2}&=&\dfrac{\partial}{\partial x}\left[2x^2y^3\cos\left(x^2y^3\right)\right]=2y^3\cos\left(x^2y^3\right)-4x^3y^6-4x^3y^6\sen\left(x^2y^3\right)\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial x\partial y}&=&\dfrac{\partial}{\partial x}\left[3x^2y^2\cos\left(x^2y^3\right)\right]=6xy^2\cos\left(x^2y^3\right)-6x^3y^5\sen\left(x^2y^3\right)\\\\
	    f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y\partial x}&=&\dfrac{\partial}{\partial y}\left[2x^2y^3\cos\left(x^2y^3\right)\right]=6xy^2\cos\left(x^2y^3\right)-6x^3y^5\sen\left(x^2y^3\right)\\\\
	    \dfrac{\partial^2 f}{\partial y^2}&=&\dfrac{\partial}{\partial y}\left[3x^2y^2\cos\left(x^2y^3\right)\right]=6x^2y\cos\left(x^2y^3\right)-6x^4y^4\sen\left(x^2y^3\right)
	\end{array}$$
	Luego, vemos que $f_{1,2}(x,y)=f_{2,1}(x,y)$.
	$$f_{1,2}(x,y)=\dfrac{\partial^2f}{\partial x \partial y}=6xy^2\cos\left(x^2y^3\right)-6x^3y^5\sen\left(x^2y^3\right)=\dfrac{\partial^2 f}{\partial y \partial x}=f_{2,1}(x,y).$$\\
	...
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x}&=&\dfrac{\partial}{\partial x}\left\{-2\cos\left[\cos\left(2x-3y\right)\sen\left(2x-3y\right)\right]\right\}\\\\
					  &=&-4\left\{\cos(2x-3y)\cos\left[\cos(2x-3y)\right]+\left[\sen(2x-3y)\right]^2\sen\left[\cos(2x-3y)\right]\right\}\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial x\partial y}&=&\dfrac{\partial}{\partial x}\left\{3\cos\left[\cos\left(2x-3y\right)\sen\left(2x-3y\right)\right]\right\}\\\\
								   &=&6\cos(2x-3y)\cos\left[\cos(2x-3y)\right]+6\left[\sen(2x-3y)\right]^2\sen\left[\cos(2x-3y)\right]\\\\
								  f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y^2}&=&\dfrac{\partial}{\partial y}\left\{-2\cos\left[\cos\left(2x-3y\right)\sen\left(2x-3y\right)\right]\right\}\\\\
								   &=&6\cos(2x-3y)\cos\left[\cos(2x-3y)\right]+6\left[\sen(2x-3y)\right]^2\sen\left[\cos(2x-3y)\right]\\\\
								   	    \dfrac{\partial^2 f}{\partial y^2}&=&\dfrac{\partial}{\partial y}\left\{3\cos\left[\cos\left(2x-3y\right)\sen\left(2x-3y\right)\right]\right\}\\\\
																					    &=&-9\left\{\cos(2x-3y)\cos\left[\cos(2x-3y)\right]+\left[\sen(2x-3y)\right]^2\sen\left[\cos(2x-3y)\right]\right\}
	\end{array}$$
	Luego, vemos que $f_{1,2}(x,y)=f_{2,1}(x,y)$.
	$$\dfrac{\partial^2f}{\partial x \partial y}=6\cos(2x-3y)\cos\left[\cos(2x-3y)\right]+6\left[\sen(2x-3y)\right]^2\sen\left[\cos(2x-3y)\right]=\dfrac{\partial^2 f}{\partial y \partial x}.$$\\
	...
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x} &=&\dfrac{1}{x-y}-\dfrac{x+y}{(x-y)^2}=\dfrac{-2y}{(x-y)^2}\\\\
	    \dfrac{\partial f}{\partial y} &=& \dfrac{1}{x-y}+\dfrac{x+y}{(x-y)^2}=\dfrac{2x}{(x-y)^2}.\\\\
	\end{array}$$
	Las derivadas parciales de segundo orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2} &=&\dfrac{\partial}{\partial x}\left[\dfrac{-2y}{(x-y)^2}\right]=\dfrac{(-2y)(2)(x-y)}{(x-y)^4}=\dfrac{4y}{(x-y)^3}\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial \partial x\partial y} &=&\dfrac{\partial}{\partial x}\left[\dfrac{2x}{(x-y)^2}\right]=\dfrac{2(x-y)^2-(2x)(2)(x-y)}{(x-y)^4}=\dfrac{-2(x+y)}{(x-y)^3}\\\\
	    f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y^2} &=&\dfrac{\partial}{\partial y}\left[\dfrac{-2y}{(x-y)^2}\right]=\dfrac{2(x-y)^2+(2y)(2)(x-y)}{(x-y)^4}=\dfrac{-2(x+y)}{(x-y)^3}\\\\
	    \dfrac{\partial^2 f}{\partial \partial y\partial x} &=&\dfrac{\partial}{\partial y}\left[\dfrac{2x}{(x-y)^2}\right]=\dfrac{(2x)(2)(x-y)}{(x-y)^4}=\dfrac{4x}{(x-y)^3}
	\end{array}$$
	Como $f_{1,2}(x,y)=f_{2,1}(x,y)$, entonces:
	$$f_{1,2}(x,y)=\dfrac{\partial^2f}{\partial x \partial y}=\dfrac{-2(x+y)}{(x-y)^3}=\dfrac{\partial^2 f}{\partial y \partial x}=f_{2,1}(x,y).$$\\
	...
	Respuesta.-\; Las derivada parcial de primer orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x} &=&\dfrac{\sqrt{x^2+y^2}-\dfrac{x^2}{\sqrt{x^2+y^2}}}{x^2y^2}=\dfrac{y^2}{\left(x^2+y^2\right)^{\frac{3}{2}}}\\\\
	    \dfrac{\partial f}{\partial y} &=&\dfrac{-x\dfrac{y}{\sqrt{x^2+y^2}}}{\left(x^2+y^2\right)^{\frac{3}{2}}}=\dfrac{-xy}{\left(x^2+y^2\right)^{\frac{3}{2}}}.\\\\
	\end{array}$$
	Las derivadas parciales de segundo orden están dadas por:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2} &=&\dfrac{\partial}{\partial x}\left[\dfrac{y^2}{\left(x^2+y^2\right)^{\frac{3}{2}}}\right]=\dfrac{-y^2\left(\dfrac{3}{2}\right)(2x)\left(\sqrt{x^2+y^2}\right)}{\left(x^2+y^2\right)^3} = \dfrac{-3xy^2}{\left(x^2+y^2\right)^{\frac{5}{2}}}\\\\
	    f_{1,2}(x,y)=\dfrac{\partial^2 f}{\partial x\partial y} &=&\dfrac{\partial}{\partial x}\left[\dfrac{-xy}{\left(x^2+y^2\right)^{\frac{3}{2}}}\right]=\dfrac{-y\left(x^2+y^2\right)^{\frac{3}{2}}+xy(2x)\left(\dfrac{3}{2}\right)\sqrt{x^2+y^2}}{\left(x^2+y^2\right)^3}=\dfrac{y\left(2x^2y-y^2\right)}{\left(x^2+y^2\right)^{\frac{5}{2}}}\\\\
	    f_{2,1}(x,y)=\dfrac{\partial^2 f}{\partial y\partial x} &=&\dfrac{\partial}{\partial y}\left[\dfrac{y^2}{\left(x^2+y^2\right)^{\frac{3}{2}}}\right]=\dfrac{2y\left(x^2+y^2\right)^{\frac{3}{2}}-y^2\left(\dfrac{3}{2}\right)(2y)\sqrt{x^2+y^2}}{\left(x^2+y^2\right)^3}=\dfrac{y\left(2x^2-y^2\right)}{\left(x^2+y^2\right)^{\frac{5}{2}}}\\\\
	    \dfrac{\partial^2 f}{\partial y^2} &=& \dfrac{\partial}{\partial y}\left[\dfrac{-xy}{\left(x^2+y^2\right)^{\frac{3}{2}}}\right]=\dfrac{-x\left(x^2+y^2\right)^{\frac{3}{2}}+xy(2y)\left(\dfrac{3}{2}\right)\sqrt{x^2+y^2}}{\left(x^2+y^2\right)^3}=\dfrac{x(xy^2-x^2)}{\left(x^2+y^2\right)^{\frac{5}{2}}}
	\end{array}$$
	Como $f_{1,2}(x,y)=f_{2,1}(x,y)$, entonces:
	$$f_{1,2}(x,y)=\dfrac{\partial^2f}{\partial x \partial y}=\dfrac{y\left(2x^2y-y^2\right)}{\left(x^2+y^2\right)^{\frac{5}{2}}}=\dfrac{\partial^2 f}{\partial y \partial x}=f_{2,1}(x,y).$$\\
	...
	\begin{enumerate}[(a)]
		...
		$$\begin{array}{rcl}
		    x\left(\dfrac{\partial z}{\partial x}\right) + y\left(\dfrac{\partial z}{\partial y}\right) &=&x\left[\dfrac{\partial}{\partial x}\left(x-2y\right)^2\right]+y\left[\dfrac{\partial}{\partial y}\left(x-2y\right)^2\right]\\\\
																    &=&x\left[2(x-2y)\right]+y\left[2(-2)(x-2y)\right]\\\\
								&=&2x^2-4xy-4xy+8y^2\\\\
								&=&2z.
		\end{array}.$$

	    \item $z=\left(z^4+y^4\right)^{1/2}$.\\\\
		Demostración.-\; Calculando se tiene,
		$$\begin{array}{rcl}
		    x\left(\dfrac{\partial z}{\partial x}\right)+y\left(\dfrac{\partial z}{\partial z}\right)&=&x\left[\dfrac{\partial}{\partial x}\left(x^4+y^4\right)^{\frac{1}{2}}\right]+y\left[\dfrac{\partial}{\partial y}\left(x^4+y^4\right)^{\frac{1}{2}}\right]\\\\
		    &=&x\left(4x^3\right)\left(\dfrac{1}{2}\right)\left(x^4+y^4\right)^{-\frac{1}{2}}+y\left(4y^3\right)\left(\dfrac{1}{2}\right)\left(x^4+y^4\right)^{-\frac{1}{2}}\\\\
		    &=&2\left(x^4+y^4\right)^{\frac{1}{2}}\\\\
		    &=&2z.
		\end{array}$$
		\vspace{.5cm}
	...
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial x} &=&\dfrac{y\left(x^2+y^2\right)^2-2(xy)(2x)\left(x^2+y^2\right)}{\left(x^2+y^2\right)^4}\\\\
					   &=&\dfrac{x^2y+y^3-4x^2y}{\left(x^2+y^2\right)^3}\\\\
					   &=&\dfrac{y^3-3x^2y}{\left(x^2+y^2\right)^3}\\\\
	\end{array}$$
	Luego, calculamos la derivada parcial de primer orden con respecto a $y$
	$$\begin{array}{rcl}
	    \dfrac{\partial f}{\partial y} &=&\dfrac{x\left(x^2+y^2\right)^2-2(xy)(2y)\left(x^2+y^2\right)}{\left(x^2+y^2\right)^4}\\\\
					   &=&\dfrac{x^3+y^2-4xy^2}{\left(x^2+y^2\right)^3}\\\\
					   &=&\dfrac{x^3-3xy^2}{\left(x^2+y^2\right)^3}\\\\
	\end{array}$$

	Ahora, calculamos las derivadas parciales de segundo orden con respecto a $x$ y $y$ y viceversa
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2} &=& \dfrac{\partial}{\partial x}\left[\dfrac{y^3-3x^2y}{\left(x^2+y^2\right)^2}\right]\\\\
					       &=&\dfrac{-6xy\left(x^2+y^2\right)^3-3\left(y^3-3x^2y\right)(2x)\left(x^2+y^2\right)^2}{\left(x^2+y^2\right)^6}\\\\
					       &=&12xy\dfrac{x^2-y^2}{\left(x^2+y^2\right)^4}\\\\
	    \dfrac{\partial^2 f}{\partial y^2} &=& \dfrac{\partial}{\partial y}\left[\dfrac{x^3-3xy^2}{\left(x^2+y^2\right)^2}\right] = \dfrac{-6xy\left(x^2+y^2\right)^3-3\left(x^3-3xy^2\right)(2y)\left(x^2+y^2\right)^2}{\left(x^2+y^2\right)^6}=-12xy\dfrac{x^2-y^2}{\left(x^2+y^2\right)^4}.\\\\
	\end{array}$$
	Finalmente, sumando las dos últimas ecuaciones se tiene:
	$$\begin{array}{rcl}
	    \dfrac{\partial^2 f}{\partial x^2}+\dfrac{\partial^2 f}{\partial y^2} &=&12xy\dfrac{x^2-y^2}{\left(x^2+y^2\right)^4}-12xy\dfrac{x^2-y^2}{\left(x^2+y^2\right)^4}\\\\
									       &=&0.
	\end{array}$$
