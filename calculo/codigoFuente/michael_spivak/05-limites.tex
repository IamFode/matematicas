...
La función $f$ se aproxima al límite $l$ cerca de $a$, si $f(x)$ se aproxima tanto como se quiera a $l$ si $x$ se aproxima suficientemente a $a$ pero es distinto de $a$. \\
...
\begin{teo}
	...
	Además, si $m\neq 0$, entonces
	\item  $\lim\limits_{x \to a} \left(\dfrac{1}{g}\right)(x) = \dfrac{1}{m}$
    \end{enumerate}
	Demostración.-\; La hipótesis significa que para todo $\epsilon > 0$ existen $\delta_1, \delta_2>0$ tales que, para todo $x$, $$si \; 0<|x-a|<\delta_1, \; \mbox{entonces} \; |f(x)-l|<\epsilon$$ 
	$$y \quad si \; 0<|x-a|<\delta_2, \; \mbox{entonces} \; |g(x)-m < \epsilon|$$
	...
	$$si \; 0<|x-a|<\delta_1, \; \mbox{entonces} \; |f(x)-l|<\dfrac{\epsilon}{2}$$
	$$y \quad si \; 0<|x-a|<\delta_2, \; \mbox{entonces} \; |g(x)-m|<\dfrac{\epsilon}{2}$$
	...
	Para demostrar $(2)$ procedemos de la misma manera, después de consultar la parte $(2)$ del lema. Si $\epsilon>0$ existen $\delta_1,\delta_2>0$ tales que, para todo $x$ $$si\; 0<|x-a|<\delta_1, \; \mbox{entonces} \; |f(x)-l|<\min\left(1,\dfrac{\epsilon}{2(|m|+1)}\right),$$ $$y\quad si\; 0<|x-a|<\delta_2,\; \mbox{entonces} \; |g(x)-m|<\dfrac{\epsilon}{2(|l|)+1}$$ Pongamos de nuevo $\delta = \min(\delta_1,\delta_2)$. Si $0<|x-a|<\delta,$ \mbox{entonces} $$|f(x)-l|<\min\left(1,\dfrac{\epsilon}{2(|m|+1)}\right) \qquad y \qquad |g(x)-m|<\dfrac{\delta}{2(|l|+1)}$$
	Así pues, según el lema, $|(f\cdot g)(x)-l\cdot m|<\epsilon$, y esto demuestra $(2)$.\\\\
	Finalmente, si $\epsilon>0$ existe un $\delta>0$ tal que, para todo $x$, $$si\; 0<|x-a|<\delta, \; \mbox{entonces} \; |g(x)-m|< \min\left(\dfrac{|m|}{2},\dfrac{\epsilon|m|^2}{2}\right)$$
...
%--------------------definición 5.2
\begin{def.}
    $\lim\limits_{x \to a^+} f(x)=l$ significa que para todo $\epsilon>0$, existe un $\delta>0$ tal que, para todo $x$, $$si \; 0<x-a<\delta, \; \mbox{entonces} \; |f(x)-l|<\epsilon$$	
    La condición $0<x-a<\delta$ es equivalente a $0<|x-a|<\delta$ y $x>a$
\end{def.}

%--------------------definición 5.3
\begin{def.}
    $\lim\limits_{x \to a^-} f(x)=l$ significa que para todo $\epsilon>0$, existe un $\delta>0$ tal que, para todo $x$, $$si \; 0<a-x<\delta, \; \mbox{entonces} \; |f(x)-l|<\epsilon$$	
\end{def.}

%--------------------definición 5.4
\begin{def.}
    $\lim\limits_{x \to \infty} f(x)=l$ significa que para todo $\epsilon>0$, existe un número $N$ grande, que, para todo $x$, $$si \; x>N, \; \mbox{entonces} \; |f(x)-l|<\epsilon$$	
\end{def.}
...
\item Demostrar que $\lim\limits_{x\to a} f(x) = \lim\limits_{h \to 0}f(a+h).$\\\\
    Demostración.-\; Sea $\lim\limits_{x\to a} f(x)$ y $g(h) = f(a+h)$. Entonces para todo $\epsilon>0$ existe algún $\delta > 0$, tal que, para todo $x$, si $0<|x-a|<\delta$, entonces $|f(x)-l|<\epsilon$. Ahora bien, si $0<|h-0|<\delta$, entonces $|(h+a)-a|<\delta$, de modo que $|f(h+a)-l|<\epsilon$. Esta desigualdad puede escribirse $|g(x)-l|<\epsilon$. Así pues, $\lim\limits_{h\to 0}g(h) = l,$ lo cual puede escribirse también $\lim\limits_{h\to 0} f(a+h) = l$. el mismo razonamiento demuestra que si $\lim\limits_{h\to 0} f(a+h) = m$, entonces $\lim\limits_{x\to a} f(x) = m.$ Así pues, existe uno cualquiera de los dos límites si existe el otro, y en este caso son iguales.\\

    Se puede demostrar de otra manera. Sea $h=x-a$, observe que $h\to 0\; \Leftrightarrow \; x-a\to 0 \; \Rightarrow \; x\to a$, entonces 
...
%--------------------11.
\item Supóngase que existe un $\delta > 0$ tal que $f(x) = g(x)$ cuando $0<|x-a|<\delta$. Demostrar que $\lim\limits_{x \to a} f(x) = \lim\limits_{x \to a} g(x)$. En otras palabras, el $\lim\limits_{x\to a}f(x)$ depende solamente de los valores de $f(x)$ cuando $x$ está cerca de $a$; este hecho se expresa a menudo diciendo que los límites poseen una propiedad local.\\\\
...
\item Definamos $\lim\limits_{x\to a} f(x) = \infty$ en el sentido de que para todo $N$ existe un $\delta>0$ tal que, para todo $x$, si $0<|x-a|<\delta$ entonces $f(x)>N$. 
	...
	%----------(a)
	\item Demostrar que $\lim\limits_{x\to 3} 1/(x-3)^2 = \infty$.\\\\ 
	    Demostración.-\; Sabiendo que $N>0$ y sea $\delta = 1/\sqrt{N}$, entonces $0<|x-3|<\delta$ implica que $(x-3)^2 < 1/N$, de donde $1/(x-3)^2>N$.\\\\ 

	%----------(b)
	\item Demostrar que si $f(x)>\epsilon>0$ para todo $x$, y $\lim\limits_{x\to a} g(x)=0,$ entonces $$\lim_{x\to a} f(x)/g(x)=\infty$$
	    Demostración.-\; Dado $N>0$, existe un $\delta>0$ tal que $0<|x-a|<\delta$ entonces $|g(x)|<\dfrac{\epsilon}{N}$, luego $$0<|x-a|<\delta \; \Longrightarrow \; \dfrac{f(x)}{|g(x)|}>\dfrac{\epsilon}{\frac{\epsilon}{N}} = N,$$
...
\item 
	...
	%----------(b)
	 \item Demostrar que $\lim_{x\to 0^+} \dfrac{1}{x} = \infty$\\\\
	     Demostración.-\; Dado $N>0$, elegir un $\delta = 1/N$. Si $0<x<\delta$, entonces $1/x >N$.\\\\

	%----------(c)	
	 \item Si $\lim\limits_{x\to \infty} f(1/x) = \infty$, entonces para todo $N$ existe algún $M$ tal que $f(1/x)<N$ para $x>M$. Escoge $M>0$. Si $0<x<1/M$, entonces $x>M$, así $f(x)>N.$. Por lo tanto $\lim\limits_{x\to 0^+} f(x) = \infty$. La demostración en dirección contraria es similar.\\\\

    \end{enumerate}

%--------------------39.
\item Hallar los siguientes límites, si existen

    \begin{enumerate}[\bfseries (i)]

	%----------(i)
	\item $\lim\limits_{x\to \infty} \dfrac{x^3 + 4x - 7}{7x^2 - x + 1} = \lim\limits_{x\to \infty} \dfrac{x + \dfrac{4}{x} - \dfrac{7}{x^2}}{7-\dfrac{1}{x}+\dfrac{1}{x^2}} = \infty$.\\\\

	%----------(ii)
	\item $\lim\limits_{x\to \infty} x(1+\sen^2 x)$.\\\\
	    Respuesta.-\; El límite esta dado desde $1\leq 1 + \sen^2x\leq 2$ para todo $x$.\\\\

	%----------(iii)
	\item $\lim\limits_{x\to \infty} x\sen^2 x.$\\\\
	    Respuesta.-\; El límite no existe, ya que $\sen^2 x$ oscila entre 0 y 1.\\\\

	%----------(iv)
	\item $\lim\limits_{x\to \infty} x^2 \sen \dfrac{1}{x} = \lim\limits_{x\to \infty} x \cdot x\sen\dfrac{1}{x} = \infty$.\\\\

	%----------(v)
	\item $\lim\limits_{x\to \infty} \sqrt{x^2 + 2x - x} = \lim\limits_{x\to \infty} \dfrac{\left(\sqrt{x^2+2x}-x\right)\left(\sqrt{x^2+2x}+x\right)}{\sqrt{x^2+2x}+x} = \lim\limits_{x\to \infty} \dfrac{2x}{\sqrt{x^2+2x}+x} = \lim\limits_{x\to \infty} \dfrac{2}{\sqrt{1+\dfrac{2}{x}}+1} = 1$.\\\\

	%----------(vi)
	\item $\lim\limits_{x\to \infty} x(\sqrt{x+2}-\sqrt{x}) = \lim\limits_{x\to \infty} \dfrac{x\left(\sqrt{x+2}-\sqrt{x}\right)\left(\sqrt{x+2}+\sqrt{x}\right)}{\sqrt{x+2}+\sqrt{x}}= \lim\limits_{x\to \infty} \dfrac{2x}{\sqrt{x+2}+\sqrt{x}} = \lim\limits_{x\to \infty} \dfrac{2}{\sqrt{\dfrac{1}{x}+\dfrac{2}{x^2}+\sqrt{1}{x}}} = \infty$ 

	%----------(vii)
	\item $\lim\limits_{x\to \infty} \dfrac{\sqrt{|x|}}{x} = \lim\limits_{x\to \infty} \dfrac{\sqrt{x}}{x}= \lim\limits_{x\to \infty} \dfrac{1}{\sqrt{x}}=0$.\\\\
...
\item
	...
	%----------(a)
	\item Hallar el perímetro de un n-ágono regular inscrito en una circunferencia de radio $r$; para las funciones trigonométricas que entren en juego.\\\\
	    Respuesta.-\; Si trazamos un circulo y un punto central $O$, dos puntos $A$ y $C$ en el perímetro de la circunferencia y un punto medio $B$  entre $A$ y $C$ trazando una linea recta, entonces el angulo $BOC$ es $\pi/n$. Luego $BC = r\sen(\pi/n)$ y $AC=2\pi \sen(\pi/n).$ Así el perímetro es $2rn \sen(\pi/n).$\\\\

	%----------(b)
	\item ¿A qué valor se aproxima este perímetro cuando $n$ es muy grande?\\\\
	    Respuesta.-\; A medida que $n$ se vuelve muy grande, esto se aproxima a $$\lim\limits_{x\to \infty} 2rx\sen\left(\dfrac{\pi}{x}\right) = \lim\limits_{x\to \infty}\pi 2r \dfrac{x}{\pi} \sen\left(\dfrac{\pi}{x}\right) = 2\pi r \alpha$$
	    donde $\alpha = \lim\limits_{x\to \infty}(\sen x)/x$ como el problema 45 ii.\\\\

    \end{enumerate}

%--------------------41.
\item Una vez ya en la  imprenta el manuscrito de la primera edición de éste libro, se me ocurrió una manera mucho más sencilla de demostrar que $\lim x^2 = a^2$, y $\lim\limits_{x\to a} x^3 = a^3$, sin pasar por todos los pasos de la página 83. Supongamos que queremos demostrar que $\lim\limits_{x\to a} x^2 = a^2,$ siendo $a>0$. Dado $\epsilon > 0$ hacemos simplemente que $\delta$ sea el mínimo de $\sqrt{a^2+\epsilon} - a$  y $a-\sqrt{a^2-\epsilon}$; entonces $|x-a|<\delta$ implica que $\sqrt{a^2-\epsilon } < x < \sqrt{a^2 + \epsilon}$, de modo que $a^2 - \epsilon < x^2 < a^2 + \epsilon$, o $|x^2-a^2|<\epsilon$. Por fortuna no llegué a tiempo para introducir estos cambios, puesto que esta demostración es totalmente falsa.¿Dónde se encuentra el fallo?.\\\\
    Respuesta.-\; 
