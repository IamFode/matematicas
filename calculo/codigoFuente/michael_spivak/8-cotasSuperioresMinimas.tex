...
\begin{enumerate}[\bfseries 1.]
	...
	\begin{enumerate}[\bfseries (a)]
		...
		\begin{center}
		    \begin{tikzpicture}
		    \begin{axis}[ticks=none,draw opacity = .5, style={
		      domain=-1:6,samples=100,smooth}, 
		      ...
		      \addplot[black,opacity=1]{gauss(2.5,.5)};
		      \addplot[mark=*] coordinates {(0.2,0)}node[below]{$a$};
		      \addplot[mark=*] coordinates {(0.7,0)}node[below]{$c$};
		      \addplot[mark=*] coordinates {(4.8,0)}node[below]{$b$};
		      \addplot[mark=*] coordinates {(4.3,0)}node[below]{$d$};
		      \addplot[mark=*] coordinates {(2.5,0)}node[below]{$x_0$};
		      \addplot[mark=*] coordinates {(.5,-.1)}node[below]{$x_1$};
		      \addplot[mark=*] coordinates {(1.1,0)}node[below]{$x_2$};
		      \addplot[] coordinates {(-1,0)}node[below]{\tiny$f(a)=f(b)=0$};
		    \end{axis}
		\end{tikzpicture}
		\end{center}
		\vspace{.5cm}
	    ...
	    %---------- (b)
	    \item Suponga que $f$ es continua en $[a,b]$ y que $f(a)<f(b)$. Demuestre que existen números $c$ y $d$ con $a\leq c<d\leq b$ tales que $f(c)=f(a)$ y $f(b)=f(b)$ y $f(a)<f(x)<f(d)$ para todo $x$ de $(c,d)$.\\\\
		Demostración.-\; 
