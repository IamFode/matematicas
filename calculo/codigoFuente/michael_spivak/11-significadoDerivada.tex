...
%\chapter{Significado de la derivada \\ \small Michael Spivak. Cálculo. \boldmath$3^{a}$ Edición. \\ Solución de ejercicios y apuntes.}
...
\begin{enumerate}[\bfseries 1.]
	...
	\begin{enumerate}[(i)]
		...
		\begin{center}
			...
			\begin{axis}[scale=.5,draw opacity =.5,samples=100,smooth, 
			  ...
			  \addplot[black,opacity=1,domain=-2:2]{x^3-x^2-8*x+1};
			\end{axis}
			  \draw(2.8,0)node[below]{Convexa};
			  \draw(.5,3.1)node[below]{Cóncava};
		    \end{tikzpicture}
		\end{center}
		\vspace{.5cm}
		...
		\begin{center}
			...
			\begin{axis}[scale=.5,draw opacity =.5,samples=100,smooth, 
			  ...
			  \addplot[black,opacity=1,domain=-1:1]{x^5+x+1};
			\end{axis}
			\draw(2.35,3)node[below]{Convexa};
			\draw(.8,0)node[below]{Cóncava};
		    \end{tikzpicture}
		\end{center}
		\vspace{.5cm}
	    ...
	    \item $f(x)=\dfrac{1}{x^5+x+1}$ en $\left[-\dfrac{1}{2},1\right]$.\\\\
		Respuesta.-\; Calculando la primera deriva, tenemos
		$$f'(x)=-\dfrac{5x^4+1}{(x^5+x+1)^2}$$
		Donde, no existen puntos críticos.
		La segunda derivada estará dada por
		$$f''(x)=\dfrac{2\left(15x^8-10x^3+1\right)}{(x^5+x+1)^3}$$
	...
	\begin{enumerate}[a)]
		...
		Por lo tanto,
		$$\dfrac{f(c)-f(d)}{c-d}\leq \dfrac{f(x_3)-f(d)}{x_3-d}\leq \dfrac{f(x_3)-f(x_2)}{x_3-x_2}.\qquad (4)$$

		Similar a lo de arriba, si $x_0<x_1<d$ y $x_1<d<c$. Entonces,
		$$\dfrac{f(c)-f(x_1)}{c-x_1}\geq \dfrac{f(c)-f(x_0)}{c-x_0}\geq \dfrac{f(x_1)-f(x_0)}{x_1-x_0},$$
		$$\mbox{y}$$
		$$\dfrac{f(c)-f(d)}{c-d}\geq \dfrac{f(c)-f(x_1)}{c-x_0}\geq \dfrac{f(d)-f(x_1)}{d-x_1},$$
		respectivamente. De esta manera:
		$$\dfrac{f(c)-f(d)}{c-d}\geq \dfrac{f(c)-f(x_1)}{c-x_1}\geq \dfrac{f(x_1)-f(x_0)}{x_1-x_0}.\qquad (5)$$

		Así, por (4) y (5) para $c,d\in[x_1,x_2]$, 
		$$\dfrac{f(c)-f(d)}{c-d}\leq M = \max\left\{\dfrac{|f(x_1)|-f(x_0)}{x_1-x_0},\dfrac{|f(x_3)-f(x_2)|}{x_3-x_2}\right\}.$$

		Por último, dado $\epsilon>0$ tomemos $0<\delta<\min\left\{\dfrac{\epsilon}{M},\dfrac{x_2-x_1}{2}\right\}$. Entonces, para $x\in(c_\delta,c+\delta)$
		$$|f(c)-f(x)|\leq |c-x|M<\dfrac{\epsilon}{M}=\epsilon.$$
		...
		Respuesta.-\; Si tuviéramos un intervalo cerrado en lugar de un intervalo abierto en la parte (a), la función $f$ podría no ser continua. Por ejemplo, consideremos al función $f:[0,1]\to \mathbb{R}$ definida por
		$$
		f(x)=\left\{
		    \begin{array}{rcl}
			x^2 & \mbox{si} & x\neq 1,\\
		...
		\right.
		$$
		Claramente $f$ es convexa en $[0,1]$.\\\\
	...
	\begin{enumerate}[(a)]
		...
		Respuesta.-\;
		\begin{enumerate}[\textit{Teorema} 1:]
