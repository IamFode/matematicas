...
\item Demostrar que si $f(x) = x$ para $x$ racional y $f(x) = -x$ para $x$ irracional, entonces $\lim\limits_{x\to a} f(x)$ no existe si $a\neq 0$.\\\\
    Demostración.-\; Para cualquier $\delta > 0$ existe $x$ con $0<|x-a|<\delta$ y $f(x)>a/2$, así como $x$ con $0<|x-a|<\delta$ y $f(x)<-a/2$. Puesto que la distinta entre $a/2$ y $-a/2$ es $a$, esto significa que no podemos tener $|f(x)-l|<a$ para todos estos $x$, cualquiera que sea el valor de $l$.\\\\
