...
\section{Propiedades de la integral de una función escalonada}
	...
	$$\int_{a}^{b} [s(x) + t(x)] dx = \int_{a}^{b} s(x) dx + \int_{a}^{b} t(x) dx$$
    \end{teo}

    %--------------------teorema 1.3
    \begin{teo}[Propiedad aditiva]
	$$\int_{a}^{b} c \cdot s(x) dx = c\int_{a}^{b} s(x) dx$$
    \end{teo}

    %--------------------teorema 1.4
    \begin{teo}[Propiedad de la linealidad]
	$$\int_{a}^{b} [c_1x(x) + c_2t(x)] dx = c_1 \int_{a}^{b} s(x) dx + c_2 \int_{a}^{b} dx$$
    \end{teo}

    %--------------------teorema 1.5
    \begin{teo}[Teorema de comparación] Si $s(x)<t(x)$ para todo $x$ de $[a,b]$, entonces $$\int_{a}^{b} s(x) dx = \int_{a}^{b} t(x) dx$$
    \end{teo}
