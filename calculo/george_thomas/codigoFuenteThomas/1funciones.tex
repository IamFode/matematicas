...
\begin{enumerate}[\bfseries a)]
    ...
    %----------b)
    \item Demuestre que existe sólo una manera de escribir $ƒ$ como la suma de una función par y una impar.\\\\
	Demostración.-\; Sea $f(x)-f(x) = 0 = \left(E_1(x) + O_1(x)\right) - \left(E(x) + O(x)\right)$ entonces $E(x)-E_1(x) = O(x) - O(x)$, luego $(E-E_1)(-x) = E(-x)-E_1(-x) = E(x)-E_1(x) = (E-E_1)(x) = E-E_1 $ ya que $E$ y $E_1$ son ambos pares. Análogamente se tiene que $O_1 - O$ es par. Por lo tanto $E-E_1$ y $O_1-O$ son ambos par e impar, por lo lo que deben ser igual a $0$, de donde por el ejercicio 11 se tiene $E_1=E$ y $O_1=O$, así la descomposición de $f$ encontrada en el inciso a) es única.\\\\
...
%--------------------13.
\item ¿Qué ocurre con la gráfica de $y=ax^2+bx+c$ cuando?

\begin{enumerate}[\bfseries a)]

    %----------a)
    \item $a$ cambia mientras que $b$ y $c$ permanecen fijas?
	Respuesta.-\; Si es $a>0$, la gráfica es una parábola que se abre hacia arriba. El aumento de a provoca un estiramiento vertical y un desplazamiento del vértice hacia el eje $y$ y hacia arriba. Si es $a<0$, la gráfica es una parábola que se abre hacia abajo. Disminuir a causa un estiramiento vertical y un desplazamiento del vértice hacia el eje $y$ y hacia abajo.\\\\

    %----------b)
    \item $b$ cambia? ($a$ y $c$ están fijas, $a\neq 0$).\\\\
	Respuesta.-\; Si es $a>0$, la gráfica es una parábola que se abre hacia arriba. Si también  $b>0$, entonces al aumentar $b$ provoca un desplazamiento del gráfico hacia abajo hacia la izquierda; si $b<0$, entonces al disminuir $b$ provoca un desplazamiento de la gráfica hacia abajo y hacia la derecha. Si es $a<0$, la gráfica es una parábola que se abre hacia abajo. Si $b>0$, aumentar b desplaza la gráfica hacia arriba y hacia la derecha. Si $b<0$, al disminuir $b$ desplaza el gráfico hacia arriba y hacia la izquierda.\\\\

    %----------c)
    \item $c$ cambia? ($a$ y $b$ están fijas, $a\neq 0$).\\\\
	Respuesta.-\; Al cambiar $c$ (para $a$ y $b$ fijos) por $\triangle c$, la gráfica se desplaza hacia arriba $c$ unidades si $\triangle c>0$, y hacia abajo $-\triangle c$ unidades si $\triangle c<0$.\\\\

\end{enumerate}

%--------------------14.
\item ¿Qué sucede con la gráfica de $y = a(x + b) 3 + c$ cuando

\begin{enumerate}[\bfseries a)]

    %----------a)
    \item $a$ cambia mientras que $b$ y $c$ permanecen fijas?.\\\\
	Respuesta.-\; Si $a>0$, la gráfica se eleva a la derecha de la línea vertical $x=-b$ y cae a la izquierda. Si $a<0$, la gráfica cae a la derecha de la línea $x=-b$ y sube a la izquierda. Si es $a=0$, el gráfico se reduce a la línea horizontal $y=c$. A medida que a aumenta $|a|$, la pendiente en cualquier punto dado $x=x_0$ aumenta en magnitud y la gráfica se vuelve más empinado. A medida que $|a|$ disminuye, la pendiente en $x_0$ disminuye en magnitud y la gráfica aumenta o disminuye gradualmente.\\\\
    
    %----------b)
    \item $b$ cambia? ($a$ y $c$ están fijas, $a \neq 0$).\\\\
	Respuesta.-\; Al aumentar $b$, el gráfico se desplaza hacia la izquierda; y al disminuir $b$ lo desplaza hacia la derecha.\\\\

    %----------c)
    \item $c$ cambia? ($a$ y $b$ están fijas, $a \neq 0$).\\\\
	Respuesta.-\; Al aumentar $c$, el gráfico se desplaza hacia arriba; al disminuir $c$ lo desplaza hacia abajo.\\\\

\end{enumerate}

%--------------------15.
\item El centro de masa de un objeto se mueve a una velocidad constante $v$, a lo largo de una recta que pasa por el origen. La siguiente figura muestra el sistema de coordenadas y la recta del movimiento. Los puntos indican las posiciones del objeto en cada segundo. ¿Por qué todas las áreas $A1, A2,..., A5$ de la figura son iguales? Al igual que en la ley de áreas iguales de Kepler (vea la sección 13.6), la recta que une el centro de masa del objeto recorre áreas iguales en tiempos iguales.\\\\
    Respuesta.-\; Cada uno de los triángulos ilustrados tiene la misma base $b=x\triangle t = v(1\; sec)$. Además, ¿la altura de cada triángulo tiene el mismo valor $h$. Así $\dfrac{1}{2}$(base) (altura) $ = \dfrac{1}{2}bh = A_1 = A_2 = A_3 = ...$. En conclusión, el objeto barre  áreas iguales en cada intervalo de un segundo.\\\\
...
\begin{enumerate}[\bfseries a)]
    ...
    %----------a)
    \item Determine la pendiente de la recta del origen al punto medio $P$ del lado $AB$ del triángulo representado en la siguiente figura $(a, b > 0).$\\\\
	Respuesta.-\; El punto medio entre $(0,b)$ y $(a,0)$ es $(a/2,b/2)$. Luego, la pendiente de la linea entre el origin y el punto medio es $\dfrac{b/2-0}{a/2-0} = \dfrac{b}{a}$\\\\

    %----------b)
    \item ¿Cuándo es $OP$ perpendicular a $AB$?\\\\
	Respuesta.-\; La pendiente de la linea $AB$ es $-\dfrac{b}{a}$, de donde el producto de las dos pendientes es $-\dfrac{b^2}{a^2}$, lo que significa que las dos lineas son perpendiculares cuando $a=b$, recuerde que dos lineas son perpendiculares cuando el producto de ambas pendientes es $-1$\\\\

\end{enumerate}

%--------------------17.
\item Considere el cuarto de círculo de radio $1$ y los triángulos rectángulos $ABE$ y $ACD$ representados en la siguiente figura. Use las fórmulas usuales del área para concluir que
    $$\dfrac{1}{2}\sen \theta \cos \theta < \dfrac{\theta}{2}<\dfrac{1}{2}\dfrac{\sen \theta}{\cos \theta}$$
    Respuesta.-\; Sabiendo que el area de $ABE = \dfrac{\sen \theta \cos \theta}{2}$, luego $\dfrac{thetar^2}{2} = \dfrac{\theta}{2}$. Por otro lado $\dfrac{AC}{AB}=\dfrac{AD}{AE} \; \Longrightarrow \; \dfrac{AC}{1}=\dfrac{1}{\cos \theta}$, sabemos que el área de $ACD = \dfrac{(AC)(AD) \sen \theta}{2} = \dfrac{1}{\cos \theta} \dfrac{\sen \theta}{2} = \dfrac{\sen \theta}{2\cos \theta}$ de donde $\sen\theta \cos \theta < \dfrac{\theta}{2}<\dfrac{1}{2} \dfrac{\sen \theta}{\cos \theta}$\\\\

%--------------------18.
\item Sea $f(x) = ax+b$ y $g(x) = cx+d$. ¿Qué condiciones deben satisfacer las constantes $abc$ y $d$ para que $(f\circ g)(x) = (g\circ f)(x)$ para todos los valores de $x$?.\\\\
    Respuesta.-\; Sea $f(g(x)) = f(cx+d) = a(cx+d) + b = acx + (ad+b)$ y $g(f(x)) = g(ax+b) = c(ax+b)+d = acx +(bc+d)$, de donde al igualar estas dos expresiones se tiene $ad+b=bc+d$ o $d(a-1)=b(c-1)$\\\\ 
