...
%--------------------75
\item[\bfseries 75.] Si $x^4\leq f(x)\leq x^2$ para $x$ en $[-1,1]$ y $x^2\leq f(x)\leq x^4$ para $x<-1$ y $x>1$. ¿en qué puntos $c$ conocemos automáticamente $\lim\limits_{x\to c} f(x)$? ¿Qué se puede decir sobre el valor del límite en esos puntos?.\\\\
    Respuesta.-\;  $c = 0, 1, -1$; el límite es $0$ en $c = 0$, y $1$ en $c = 1, -1$.\\\\

%--------------------76.
\item Suponga que $g(x) \leq f(x) \leq h(x)$ para toda $x \neq 2$, y suponga que
